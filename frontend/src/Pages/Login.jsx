...
const Login = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='min-h-[80vh] flex items-center'>
      <div className='flex flex-col gap-3 m-auto items-start p-8 min-w-[340px] sm:min-w-[340px] border rounded-xl shadow-lg text-zinc-600'>
        ...
        {
          state === 'Sign Up' ?
          <p>Already have an account? <span onClick={()=> setState('Login')} className='text-primary hover:underline cursor-pointer'>Login Here</span></p>:
          <p>Create an new account? <span onClick={()=> setState('Sign Up')} className='text-primary hover:underline cursor-pointer'>click here</span></p>
        }
      </div>
    </form>
  )
