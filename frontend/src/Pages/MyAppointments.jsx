          ...
          <div className='rounded-2xl border border-gray-200 bg-white p-4 shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-lg sm:p-5' key={item._id}>
            <div className='flex flex-col gap-4 sm:flex-row sm:items-start'>
              ...
              <div className='flex shrink-0 flex-col gap-2 sm:items-end'>
                {!item.cancelled && item.payment && !item.isCompleted &&<button className='sm:min-w-48 py-2 border rounded border-green-50 text-stone-500' >Paid</button>}
                {item.cancelled !== true && !item.payment && !item.isCompleted && <button onClick={()=>appointmentRazorpay(item._id)} className='w-40 rounded-full border border-primary py-2 text-sm font-medium text-primary transition hover:bg-primary hover:text-white'>
                  Pay Online
                </button>}
                {item.cancelled !== true && !item.isCompleted && <button onClick={()=>cancelAppointment(item._id)} className='w-40 rounded-full border border-red-400 py-2 text-sm font-medium text-red-400 transition hover:bg-red-400 hover:text-white'>
                  Cancel Appointment
                </button>}
                {item.cancelled === true && !item.isCompleted && <button className='sm:min-w-48 py-2 border border-red-500 rounded text-red-500'>Appointment Cancelled</button>}
